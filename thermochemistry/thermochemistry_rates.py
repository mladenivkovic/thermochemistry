...
class rates(object):
    ...
    def A_Hp(self, T):
        ...
        return 8.40e-11 / np.sqrt(T) * (T * 1e-3) ** (-0.2) * 1.0 / (1.0 + (T * 1e-6) ** 0.7)
    ...
    def A_Hepp(self, T):
        ...
        Recombination rate for He++ in units of cm^3 s^-1
        """
        return 3.36e-10 / np.sqrt(T) * (T * 1e-3) ** (-0.2) * 1.0 / (1.0 + (T * 1e-6) ** 0.7)

    def G_H0(self, T):
        """
        collisional ionization rate for H0 in units of cm^3 s^-1
        """
        return 5.85e-11 * np.sqrt(T) * np.exp(-157809.1 / T) * 1.0 / (1.0 + np.sqrt(T * 1e-5))

    def G_He0(self, T):
        """
        collisional ionization rate for He0 in units of cm^3 s^-1
        """
        return 2.38e-11 * np.sqrt(T) * np.exp(-285335.4 / T) * 1.0 / (1.0 + np.sqrt(T * 1e-5))
