...
def BT(nu, T):
    ...
    """
    res =  8. * np.pi * h_planck * nu**3 / c**3 /(np.exp(h_planck * nu / kB / T) - 1.)
...
if not skip_plots:
    ...
    ax.plot([1., 1.], [0., peakval * 1.01], label=r"$\nu_{max}$", c="k")
...
def integrate_specrum(nu_start, nu_end, npoints, T):
...
for i, n in enumerate(npoints):
    for j, end in enumerate(nu_ends):
        ...
        res = integrate_specrum(1*unyt.Hz, end*nu_peak, n, T)
        results[i][j] = res

print()
topline = "{0:20} || {1:22.3e} | {2:22.3e} | {3:22.3e}".format("nu_end/nu_peak = ", nu_ends[0], nu_ends[1], nu_ends[2])
print(topline)
for char in topline:
    print("-", end="")
print()
...
for i, n in enumerate(npoints):
    nextline = "n = {0:16d} || {1:12.6e} | {2:12.6e} | {3:12.6e}".format(n, results[i][0], results[i][1], results[i][2])
    print(nextline)
